...
int main()
{
  ...
  Mat img_src = imread(file_src, 1); // 入力画像の読み込み
  Mat img_bkg = imread(file_bkg, 1); // 背景画像の読み込み
  ...
  // カラー画像をグレースケール画像に変換
  cvtColor(img_src, img_fg, COLOR_BGR2GRAY);
  cvtColor(img_bkg, img_bg, COLOR_BGR2GRAY);
  // 背景画像との差分画像を計算
